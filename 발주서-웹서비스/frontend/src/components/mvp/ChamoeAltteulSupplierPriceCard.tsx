...
import {
  downloadBlob,
  downloadChamoeAltteulSupplierPriceMonitorWorkbook,
  fetchChamoeAltteulSupplierPriceMonitor,
...
function ChamoeAltteulSupplierPriceCard() {
  ...
  const loadData = async (asRefresh = false) => {
    try {
      ...
      setError(null);
      setData(await fetchChamoeAltteulSupplierPriceMonitor());
    } catch (e) {
      setError(e instanceof Error ? e.message : '성주참외 알뜰과 공급가 비교를 불러오지 못했습니다.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const downloadWorkbook = async () => {
    try {
      setDownloading(true);
      setError(null);
      const result = await downloadChamoeAltteulSupplierPriceMonitorWorkbook();
      downloadBlob(result.blob, result.filename);
      if (result.stats) {
        setData(result.stats as unknown as SupplierPriceMonitorData);
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : '성주참외 알뜰과 비교 엑셀 다운로드에 실패했습니다.');
    } finally {
      ...
      <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <div className="mb-2 flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-amber-600">Supplier Alert</p>
              <h3 className="text-xl font-black text-slate-900">성주참외 알뜰과 공급가</h3>
            </div>
          </div>
          <p className="text-sm leading-6 text-slate-500">
            전날 저장된 제주다팜 성주참외 알뜰과 공급가와 오늘 관리자 사이트 공급가를 비교합니다.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center rounded-xl bg-amber-500 px-4 py-2 text-sm font-bold text-white transition hover:bg-amber-400 disabled:opacity-50"
          >
                ...
                <div key={row.option_name} className={`rounded-2xl border p-4 ${style.row}`}>
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{row.sheet_name} 시트 {row.cell} 셀에 최신 공급가로 저장됩니다.</p>
                    </div>
...
export default ChamoeAltteulSupplierPriceCard;
