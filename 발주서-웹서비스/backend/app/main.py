...
# 발주 마감(KST). 이 시각 이후 생성분은 '늦은 발주'로 로그에만 표시한다 — 기록은 한다.
#
# 과거엔 마감 이후 생성분을 아예 기록하지 않았다(조회·미리보기로 간주). 그 이유는
# 마감 후 생성을 기록하면 다음 영업일 자동제외가 그 주문들을 '이미 발주됨'으로
# 걸러 조용히 누락시키기 때문이었다(2026-07-10 토스 백도 최종화 건).
# 하지만 그 침묵 누락은 이후 '제외된 주문(받는분) 표시'로 이미 막혔고, 대신 반대편
# 사고가 터졌다 — 2026-08-17 16:17 생성한 제주다팜 실제 발주(청사과 90·콜라비 1·
# 미니밤호박 41)가 마감 이후라 미기록되어, 다음날 132건이 통째로 중복 발주됐다.
#
# 시각만으로 '실제 발주'와 '미리보기'를 구분할 수 없다. 둘 중 하나를 골라야 한다면
# 기록하는 쪽이 안전하다: 잘못 기록되면 다음날 제외 목록에 이름이 떠서 눈에 보이고
# 체크박스를 꺼서 되살릴 수 있지만, 기록을 빠뜨리면 거래처로 중복 발주가 나간다.
...
async def _record_issued(section: str, filename: str, *stats_list: dict, extra_ids: list[str] | None = None) -> None:
    ...
    생성 시각과 무관하게 항상 기록한다. 마감 이후 생성분도 실제 발주일 수 있어
    빠뜨리면 다음 영업일에 중복 발주가 나간다(2026-08-17 제주다팜 132건 사고).
    """
    now_kst = datetime.now(timezone(timedelta(hours=9)))
    if now_kst.time() > _ISSUE_RECORD_CUTOFF:
        logger.info(
            "발주 마감(10:30 KST) 이후 생성 → 발주 이력에 기록함(다음날 중복 방지): "
            "section=%s file=%s (%s)",
    ...
    try:
        ...
        if collect_dates:
            try:
                ...
                toss_baekdo_entries = toss_jeju.get("baekdo", [])
...
async def send_order_email(
    ...
    supplier: str = Form("haedal"),
    _token: dict = Depends(verify_token),
):
    """업로드한 발주서 파일(1개 이상)을 발주처 메일로 첨부 발송.

    supplier=haedal(기본) → farmers2022@naver.com,
    supplier=biseller → naeun_order@naver.com (비셀러/리앤유커머스).
    ...
    result = email_service.send_order_files_email(attachments, supplier)
    if not result.get("sent"):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=f"이메일 발송 실패: {result.get('error')}",
        )
    logger.info(f"발주서 수동 이메일 발송({supplier}): {[name for _, name in attachments]}")
...
@app.post("/api/process/biseller-tracking")
async def process_biseller_tracking(
    delivery_file: UploadFile = File(...),
    tracking_file: UploadFile | None = File(None),
    tracking_file2: UploadFile | None = File(None),
    user: dict = Depends(verify_token),
):
    try:
        delivery_bytes = await delivery_file.read()
        replies = []
        for upload in (tracking_file, tracking_file2):
            if upload:
                replies.append(await upload.read())
        output_bytes, filename, stats = biseller_tracking.process(delivery_bytes, replies)
        logger.info(f"비셀러 운송장 입력 완료: {stats}")
        return make_excel_response(output_bytes, filename, stats)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.exception("비셀러 운송장 입력 중 오류")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"처리 중 오류가 발생했습니다: {str(e)}",
        )
